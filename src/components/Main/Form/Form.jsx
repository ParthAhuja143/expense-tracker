...
const Form = () => {
    ...
    useEffect(() => {
      if(segment){
          ...
          segment.entities.forEach((e) => {
            ...
            const category = `${e.value.charAt(0)}${e.value.slice(1).toLowerCase()}` // SALARY = Salary
    ...
    return (
        <Grid container spacing ={2}>
            ...
            <Grid item xs = {12}>
                <Typography color = 'primary' align = 'center' variant = 'subtitle2' gutterBottom>
                    {segment ?  (segment.words.map((w) => w.value).join(" ") ) : 'No Voice Input' }
                </Typography>
            </Grid>
            <Grid item xs = {6} >
            ...
            <Grid item xs = {6}>
                <TextField type = 'number' label = 'Amount(INR)' fullWidth value = {formData.amount} onChange = {(e) => setFormData({...formData , amount : e.target.value})} />
            </Grid>
            <Grid item xs = {6}>
